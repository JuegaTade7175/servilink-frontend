...
import type { Booking, BookingStatus, Professional, Payment, BookingConfirmation, Review, PaymentMethod, ServiceItem } from '../types';

// ─── cn helper ───────────────────────────────────────────────────────────────
function cn(...c: (string | false | undefined | null)[]) { return c.filter(Boolean).join(' '); }

// ─── Helpers ──────────────────────────────────────────────────────────────────
function fmtDate(s: string) { return new Date(s).toLocaleDateString('es-PE', { day:'2-digit', month:'short', year:'numeric' }); }
function fmtDateTime(s: string) { return new Date(s).toLocaleDateString('es-PE', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }); }
...
function Avatar({ name, url }: { name: string; url?: string }) {
  ...
  return <div className={cn('w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 ring-2 ring-white/10', color)}>{initials}</div>;
}
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
      ...
      <div className={cn('relative w-full bg-[#1a1b2e] border border-[#252640] rounded-2xl shadow-2xl overflow-hidden flex flex-col', wide ? 'max-w-2xl' : 'max-w-lg')} style={{ maxHeight: '90vh' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#252640] flex-shrink-0">
          <h2 className="font-bold text-sm text-[#e8e9f3]" style={{ fontFamily: 'var(--font-display)' }}>{title}</h2>
...
function Toast({ msg, type }: { msg: string; type: 'success'|'error'|'info' }) {
  const cfg = { success:'bg-emerald-500/20 border-emerald-500/40 text-emerald-300', error:'bg-red-500/20 border-red-500/40 text-red-300', info:'bg-[#6c63ff]/20 border-[#6c63ff]/40 text-[#a09bff]' }[type];
...
function CreateModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (b: Booking) => void }) {
  ...
  useEffect(() => {
    ...
    setLoadingPros(true);
    professionalsApi.nearby(-12.0464, -77.0428, 30).then(setPros).catch(() => {}).finally(() => setLoadingPros(false));
  }, [open]);
  ...
  const submit = async () => {
    ...
    try {
      const b = await bookingsApi.create({ professionalId: pro.id, serviceId: svc.id, scheduledAt: form.scheduledAt, address: form.address, description: form.description });
      onCreated(b); onClose();
            ...
            <div key={key} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-1 flex-1">
                <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all',
                  done ? 'bg-[#6c63ff] border-[#6c63ff] text-white' : active ? 'border-[#6c63ff] text-[#6c63ff]' : 'border-[#252640] text-[#6b6d8a]')}>
                  {done ? '✓' : i + 1}
                </div>
                <span className={cn('text-[10px] font-semibold', active ? 'text-[#6c63ff]' : done ? 'text-[#e8e9f3]' : 'text-[#6b6d8a]')}>{label}</span>
              </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'pro' && (
          <motion.div key="pro" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:.18 }} className="space-y-3">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Buscar profesional o especialidad..."
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
              {loadingPros ? (
                ...
                <p className="text-center text-[#6b6d8a] text-sm py-8">Sin resultados</p>
              ) : filtered.map(p => (
          ...
          <motion.div key="service" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:.18 }} className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-[#6c63ff]/10 rounded-xl border border-[#6c63ff]/20">
              ...
              <div className="flex-1">
                <div className="font-semibold text-sm">{pro.userName}</div>
                <div className="text-xs text-[#6c63ff]">{pro.specialty}</div>
              </div>
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {pro.services.length === 0 ? (
                <p className="text-center text-[#6b6d8a] text-sm py-6">Este profesional no tiene servicios registrados</p>
              ) : pro.services.map(s => (
        ...
        {step === 'details' && pro && svc && (
          <motion.div key="details" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:.18 }} className="space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea value={form.description} onChange={e => setForm(p=>({...p, description:e.target.value}))} rows={3} placeholder="Describe brevemente qué necesitas..."
...
// ─── Payment Modal ────────────────────────────────────────────────────────────
function PaymentModal({ open, onClose, booking, onPaid }: { open:boolean; onClose:()=>void; booking:Booking|null; onPaid:(p:Payment)=>void }) {
              ...
              {PAYMENT_METHODS.map(m => (
                <button key={m.value} onClick={() => setMethod(m.value)}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-[#6b6d8a]">{m.desc}</div>
                  </div>
        ...
        {noConf ? (
          <>
            <div className="text-center py-4">
              ...
              <p className="text-sm text-[#6b6d8a]">Genera un código de 6 dígitos que el profesional deberá introducir para confirmar la cita.</p>
            </div>
            <Btn variant="primary" size="lg" className="w-full" loading={generating} onClick={generate}>Generar código de confirmación</Btn>
          </>
          ...
          <>
            <div className="bg-[#13141f] rounded-xl p-5 border border-[#252640] text-center">
              ...
              <div className="text-4xl font-black tracking-[0.35em] text-[#6c63ff] my-3 font-mono">
                {confirmation.confirmationCode}
              </div>
              <p className="text-xs text-[#6b6d8a]">Válido hasta: {fmtDateTime(confirmation.expiresAt)}</p>
              <div className="mt-2">
                <span className="text-[10px] uppercase tracking-wider font-bold bg-amber-500/15 text-amber-400 border border-amber-500/30 rounded-full px-2 py-0.5">Pendiente</span>
              </div>
            </div>
...
// ─── Review Modal ─────────────────────────────────────────────────────────────
function ReviewModal({ open, onClose, booking, onReviewed }: { open:boolean; onClose:()=>void; booking:Booking|null; onReviewed:(r:Review)=>void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Dejar reseña">
      {booking && (
        <div className="space-y-5">
          <p className="text-sm text-[#6b6d8a] text-center">¿Cómo calificarías a <span className="font-semibold text-[#e8e9f3]">{booking.professionalName}</span>?</p>
          <div className="flex justify-center gap-2">
            {[1,2,3,4,5].map(s => (
              <button key={s} onClick={() => setRating(s)} className={cn('text-4xl transition-all hover:scale-110', s <= rating ? 'text-amber-400' : 'text-white/15')}>★</button>
            ))}
          </div>
...
function DetailPanel({ booking, onUpdated }: { booking: Booking; onUpdated: (b: Booking) => void }) {
  ...
  const changeStatus = async (status: string) => {
    setUpdating(status);
    try { const u = await bookingsApi.updateStatus(booking.id, status); onUpdated(u); showT(`Estado: ${STATUS_CFG[status]?.label ?? status}`); }
  ...
  const actions: ActionDef[] = {
    PENDING:     [{label:'Confirmar',icon:'✅',status:'CONFIRMED',variant:'success'},{label:'Cancelar',icon:'❌',status:'CANCELLED',variant:'danger'}],
    CONFIRMED:   [{label:'Iniciar',icon:'🔧',status:'IN_PROGRESS',variant:'primary'},{label:'Cancelar',icon:'❌',status:'CANCELLED',variant:'danger'}],
    ...
    <div className="h-full overflow-y-auto">
      ...
      <div className="p-6 space-y-5 max-w-2xl mx-auto">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-black text-xl text-[#e8e9f3]" style={{fontFamily:'var(--font-display)'}}>{booking.serviceName}</h2>
        ...
        {actions.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {actions.map(a => (
                <Btn key={a.status} variant={a.variant} size="sm" icon={a.icon} loading={updating===a.status} onClick={() => changeStatus(a.status)}>{a.label}</Btn>
              ))}
            </div>
          </div>
        )}
            ...
            {existingReview ? (
              <div className="p-4 bg-[#13141f] rounded-xl border border-[#252640]">
                <div className="flex justify-between mb-2">
                  <Stars r={existingReview.rating} />
                  <span className="text-[10px] text-[#6b6d8a]">{fmtDate(existingReview.createdAt)}</span>
                </div>
      ...
      <ConfirmationModal open={showConf} onClose={()=>setShowConf(false)} booking={booking} confirmation={confirmation} onRefresh={()=>{loadExtra();showT('Código actualizado');}} />
...
export default function BookingsPage() {
  ...
  const showT = (msg: string, type: 'success'|'error'='success') => { setToast({msg,type}); setTimeout(()=>setToast(null),3000); };
  ...
  const counts = bookings.reduce<Record<string,number>>((a,b)=>{ a[b.status]=(a[b.status]??0)+1; a['ALL']=(a['ALL']??0)+1; return a; },{});
            ...
            {STATUS_FILTERS.map(({value,label,icon}) => {
              ...
              return (
                <button key={value} onClick={()=>setFilter(value)}
                  ...
                  {icon} {label}
                  {count>0 && <span className={cn('text-[9px] rounded-full w-4 h-4 flex items-center justify-center font-bold', active?'bg-white/20':'bg-[#252640] text-[#6b6d8a]')}>{count>9?'9+':count}</span>}
                </button>
              );
            })}
          </div>
        ...
        <AnimatePresence mode="wait">
          {selected ? (
            ...
            <motion.div key="empty" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="h-full flex flex-col items-center justify-center gap-4 text-center p-8">
              ...
              <p className="text-sm text-[#6b6d8a] max-w-sm">Elige una reserva del panel izquierdo para ver detalles, gestionar pagos, confirmar la cita y dejar reseñas.</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
